...
def concat_crash_files(path, final_file_name):
    ...
    for file_name in file_list:
        ...
        if "collision_date" in df.columns:
            df["accident_date"] = pd.to_datetime(df["collision_date"], format="%Y-%m-%d")
...
for state_name in ["CA"]:
    ...
    os.makedirs(data_path + "/Accidents/" + state_name,exist_ok=True)
    concat_crash_files(f"{data_path}/raw/Accidents/{state_name}/", data_path + "/Accidents/" + state_name + "/" + state_name + "_crash.pkl")
