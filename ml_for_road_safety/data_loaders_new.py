...
class TrafficAccidentDataset:
    ...
    def __init__(self, state_name = "MA", data_dir = "./data",
        ...
        self.state_data_dir = self.data_dir + "/" + self.state_name
        # download dataset
    ...
    def download_dataset(self):
        ...
        print("number of files to download", len(files_list))
        for file in files_list:
            filename = file["dataFile"]["filename"]
            file_id = file["dataFile"]["id"]
            print("working on ",filename)
            if (self.state_name in filename) and (".zip" in filename):
                response = data_api.get_datafile(file_id)
                os.makedirs(self.state_data_dir,exist_ok=True)
                with open(os.path.join(self.state_data_dir, filename), "wb") as f:
                    f.write(response.content)
                    unzip_file(os.path.join(self.state_data_dir, filename), self.state_data_dir)
                    if not os.path.exists(os.path.join(self.state_data_dir, self.state_name)):
                        print("Moving files...")
                        os.system(f"mv {self.state_data_dir}/ML4RoadSafety_graphs_{self.state_name}/{self.state_name} ./data")
    ...
    def load_monthly_data(self, year=2022, month=1):
        ...
        node_features = node_features[["node_id","lat","lon","tavg", "tmin", "tmax", "prcp", "wspd", "pres"]]
        ...
        if os.path.exists(edge_feature_dir):
            ...
            print(edge_feature_dict.keys())
            # for key in ['street_sign']: edge_features = torch.stack(edge_features, dim=1)
            column_values = edge_feature_dict['street_sign'].coalesce().values()
    ...
    def load_yearly_data(self, year=2022):
        ...
        edge_feature_dict = torch.load(edge_feature_dir)
        edge_indices =  edge_feature_dict['street_sign'].coalesce().indices()
        edge_weights = edge_feature_dict['street_sign'].coalesce().values()
        ...
        for month in range(1, 13):
            ...
            month_node_features = month_node_features[["node_id","lat","lon","tavg", "tmin", "tmax", "prcp", "wspd", "pres"]]
